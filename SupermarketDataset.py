...
# Load data
data_path = "Data/SupermarketDataset/SupermarketDataset.csv"
...
# Perform aggregation by user_id
aggregated_data = data.groupby('user_id').agg({
    'order_id': 'nunique',         # Count the number of unique orders per user
    'order_number': 'max',         # Get the maximum order number for each user
    'order_dow': 'mean',           # Get the mean order day of week for each user
    'order_hour_of_day': 'mean',   # Get the mean order hour of day for each user
    'days_since_prior_order': 'mean', # Get the mean days since prior order for each user
    'product_id': 'nunique',       # Count the number of unique products ordered by each user
    'add_to_cart_order': 'mean',   # Get the mean add to cart order
    'reordered': 'sum',            # Sum the number of reordered items
    'department_id': 'nunique',    # Count the number of unique departments for each user
    'department': lambda x: x.mode()[0],  # Get the most common department for each user
    'product_name': lambda x: x.mode()[0] # Get the most common product name for each user
}).reset_index()

# Rename the columns to something more descriptive
aggregated_data.columns = [
    'user_id', 'unique_orders', 'max_order_number', 'mean_order_dow', 'mean_order_hour_of_day',
    'mean_days_since_prior_order', 'unique_products', 'mean_add_to_cart_order',
    'total_reordered', 'unique_departments', 'most_common_department', 'most_common_product_name'
]

# Drop the first, second, and last columns
aggregated_data = aggregated_data.drop(columns=['user_id','most_common_department', 'most_common_product_name'])
...
"""
output :
    Shape of the original dataset: (2019501, 12)
    Shape of the original dataset after preprocessing: (98918, 9)
    
    Optimal number of clusters: 2
    
    Top features: ['max_order_number', 'mean_days_since_prior_order', 'total_reordered', 'unique_products']
    Shape of the reduced dataset: (98918, 4)
    
    KMeans Clustering Evaluation
    Original Dataset: Silhouette Score: 0.2490813558183269, DBI: 1.5859918784239833, CHI: 34777.80695217403, k: 2
    Reduced Dataset: Silhouette Score: 0.4610117929944613, DBI: 1.0248674935056983, CHI: 59174.04179187966, k: 2
    Reduced Dataset with Synthetic Data: Silhouette Score: 0.4385893483446598, DBI: 1.0732566061329376, CHI: 119391.19625525163, k: 2
"""